...
/**
 * Classe CreateBrewery
 * Insertion de la nouvelle brasserie dans la database
 */

public class CreateBrewery extends AsyncTask<BreweryEntity, Void, Void> {
    private Application app;
    private Exception exception;

    public CreateBrewery(Application app) {
        this.app = app;
    }
    ...
    protected Void doInBackground(BreweryEntity...breweryEntities) {
        try {
            for(BreweryEntity brewery: breweryEntities)
                ((RemembeerApp) app).getDatabase().breweryDao().insert(brewery);
        } catch (Exception e) {
            exception = e;
        }
        return null;
