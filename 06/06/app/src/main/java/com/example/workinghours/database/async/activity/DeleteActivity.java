...
public class DeleteActivity extends AsyncTask<ActivityEntity, Void, Void> {
    private Application application;
    private OnAsyncEventListener callback;
    private Exception exception;

    public DeleteActivity(Application app, OnAsyncEventListener listener){
    ...
    protected Void doInBackground(ActivityEntity...params){
        try{
            for(ActivityEntity activity : params)
                ((BaseApp) application).getDatabase().activityDao().delete(activity);
