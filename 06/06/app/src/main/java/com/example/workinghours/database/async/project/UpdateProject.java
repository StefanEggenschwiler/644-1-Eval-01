...
public class UpdateProject extends AsyncTask<ProjectEntity, Void, Void> {

    private Application application;
    private OnAsyncEventListener callback;
    private Exception exception;

    public UpdateProject(Application app, OnAsyncEventListener listener){
    ...
    protected Void doInBackground(ProjectEntity...params){
        try{
            for(ProjectEntity project : params)
                ((BaseApp) application).getDatabase().projectDao().update(project);
